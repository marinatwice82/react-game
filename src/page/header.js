...
import './header.css';
import soundfile from '../audio/music.mp3';
...
class Header extends React.Component {
    ...
    playHumans(){
        if(MyContext._currentValue.rival == 'Computer'){
         ...
         let btn_computer = document.querySelector('.btn_computer');
            btn_computer.style.background = '#4a9396';  
    ...
    playComputer(){
        ...
        let btn_player = document.querySelector('.btn_player');
                btn_player.style.background = ' #4a9396';  
			...
			<div className = "header">
        		...
        		<div className="btn_start">
                    <div className = "sound">
                        ...
                        <button className = "sound-btn" onClick={this.play_sound}>{this.state.isSound ? 'Stop' : 'Music'}</button>
        		      </div>
                </div>
			</div>
		);
